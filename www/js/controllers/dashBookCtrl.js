angular.module('starter.controllers.dashBookCtrl', []).controller('DashBookCtrl',function ($scope, $ionicTabsDelegate, $calendar, $http, $httpPsd, $timeout, $qupload, $operation, $ionicPopover, $showAlert, $cordovaDatePicker, $stateParams, $ionicPopup, $state, $ionicLoading, $ionicScrollDelegate, $T, ionicDatePicker) {
  ...
  $scope.$on('$ionicView.enter', function () {
    ...
    if ($stateParams.type == 1) {
      ...
      console.log('宴会啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊');
    } else {
  ...
  $scope.areaSuccess = function (data) {
    sessionStorage.setItem('resvOrderTypes', JSON.stringify(data.resvOrderTypes));
    $scope.tableAreas = data.tableAreas;
    var tableAreasLength = $scope.tableAreas.length > 5 ? 290 : ($scope.tableAreas.length + 1) * 53;
  ...
  $scope.seatSuccess = function (data) {
    ...
    if(localStorage['scrollTop']!= 0){
      $ionicScrollDelegate.$getByHandle('tableScroll').scrollTo(0,localStorage['scrollTop'],false);
    }
  };
  ...
  $scope.colorArr = ['#DA4C39', '#037b5f', '#E5AD51', '#646d6b', '#0060d2', '#B287D8', '#9A8787', '#9A32CD', '#FF69B4', '#EEEE00'];
  ////////////////////////////////////////弹出框

  var checkUp = function (arr, state) { // 校验所选桌位上默认订单状态是否为state
    if (arr[0]) {
  ...
  var checkTableOrders = function (arr, state) { // 校验所选桌位上是否有状态为state的订单
    if (arr[0]) {
      for (var i = 0; i < arr.length; i++) {
        var tableorders = arr[i].getAttribute('data-orders')?JSON.parse(arr[i].getAttribute('data-orders')):[];
  ...
  var checkOrdersAppuser = function (arr, id) { // 校验所选桌位上是否有appuserid为id的订单
    if (arr[0]) {
      for (var i = 0; i < arr.length; i++) {
        var tableorders = arr[i].getAttribute('data-orders')?JSON.parse(arr[i].getAttribute('data-orders')):[];
        for(var a=0;a<tableorders.length;a++){
  ...
  $scope.chooseOrder = function($event, type){ // 翻台订单弹窗上选择 type==1 加桌 type==2 餐前确认
    ...
    if(type==2&&obj.getAttribute('data-appUserId') != $scope.info.id){ // 餐前确认
      $ionicLoading.show({
    ...
    if($scope.info.operationType != 1 && obj.getAttribute('data-appUserId') != $scope.info.id){
      $ionicLoading.show({
  ...
  $scope.isTableOrdersMuti = function(){ // 校验单个桌位上是否有翻台订单
    var ismuti = false;
    ...
    $scope.table.map(function(item) {
      ...
      if(tableorders.length>1){ ismuti = true }
      $scope.alltableorders = $scope.alltableorders.concat(tableorders)
    })
    return ismuti
  }
  $scope.orderHtml = function(item, type){ // 翻台订单弹窗dom
    if ($scope.info.showTableVipname == false) {
    ...
    return `<button class="button-block btn-order" ng-click="chooseOrder($event,${type})" data-status="${item.status}" data-resvOrder="${item.resvOrder}" data-batchno="${item.batchNo}" data-resvNum="${item.resvNum}" data-tableName="${item.tableName}" data-tableAreaId="${item.tableAreaId}" data-tableAreaName="${item.tableAreaName}" data-seat="${item.tableId}" data-maxPeopleNum="${item.maxPeopleNum}" data-destTime="${item.destTime}" data-flag="${item.flag}" data-appUserId="${item.appUserId}" ng-style="bgColor[${item.status-1}]">
      <span class="disable-pointer-events">${item.tableName}</span>
      <span class="disable-pointer-events" style="float: right;">${item.vipName}</span><br>
      <span class="disable-pointer-events" style="margin-right:5px;">${item.resvNum}人</span>
      ...
      <span class="disable-pointer-events" style="float: right;">
        ...
        <i ng-show="${item.flag}" style="font-size:15px;color: ${$scope.colorArr[item.flag%10]};" class="vertical-middle icon ion-ios-infinite"></i>
        <i ng-show="${item.status==0&&item.nextStatus!=0&&$scope.info.kbc}" class="icon ion-alert" style="color:#DA4C39;"></i>
        <i ng-show="${item.remark.length>0 || item.orderTag.length>0 || item.facilities.length>0}" style="font-size:15px;color: #DA4C39;" class="vertical-middle icon ion-chatbubble-working"></i>
        <span ng-show="${item.peicai*1}" ng-class={'purple':(item.picUrl||item.isDc),'red':(!(item.picUrl||item.isDc))}>{{'配'|T}}</span>
        <i ng-show="${item.confirm*1}" style="font-size:12px;" ng-class={'purple':{{item.confirm!=1}},'red':{{item.confirm==1}}} class=" icon ion-ios-star"></i>
      </span><br>
      <span class="disable-pointer-events">${item.appUserName=='无'?'预订台':item.appUserName}</span>
    </button>`
  }
  var change = function ($event, num) {
    ...
    if (num == 1) {
      ...
      if (txt == "退订" && checkTableOrders($scope.table, 1) && ($scope.tuiding || ($scope.canSubmit && $scope.tuidingSubmit))) {
        ...
        $scope.unBookFun = function(){
          ...
          if(!checkOrdersAppuser($scope.table, $scope.info.id) && $scope.canSubmit){
                ...
                onTap: function () {
                  ...
                  if($scope.chooseOrders.length>0){
                    for (var i = 0; i < $scope.chooseOrders.length; i++) {
                      checkedTables[i] = {};
                      checkedTables[i].tableAreaId = $scope.chooseOrders[i].getAttribute("data-tableAreaId");
                      checkedTables[i].tableAreaName = $scope.chooseOrders[i].getAttribute("data-tableAreaName");
                      checkedTables[i].tableId = $scope.chooseOrders[i].getAttribute("data-seat");
                      checkedTables[i].tableName = $scope.chooseOrders[i].getAttribute("data-tableName");
                      checkedTables[i].resvOrder = $scope.chooseOrders[i].getAttribute("data-resvOrder");
                      checkedTables[i].maxPeopleNum = $scope.chooseOrders[i].getAttribute("data-maxPeopleNum");
                    }
                  }else{
                    for (var i = 0; i < $scope.table.length; i++) {
                      checkedTables[i] = {};
                      checkedTables[i].tableAreaId = $scope.table[i].getAttribute("data-tableAreaId");
                      checkedTables[i].tableAreaName = $scope.table[i].getAttribute("data-tableAreaName");
                      checkedTables[i].tableId = $scope.table[i].getAttribute("data-seat");
                      checkedTables[i].tableName = $scope.table[i].getAttribute("data-tableName");
                      checkedTables[i].resvOrder = $scope.table[i].getAttribute("data-resvOrder");
                      checkedTables[i].maxPeopleNum = $scope.table[i].getAttribute("data-maxPeopleNum");
                    }
                  }
                  if(!$scope.canSubmit){
                    $scope.unBookData = {
                    ...
                    $scope.showLoading();
                    $operation.statusUnBook($scope.unBookData, $scope.operationSuccess.unBook, $scope.error);
                  }else{
                    var orderInfo = $scope.chooseOrders.length>0?$scope.chooseOrders[0]:$scope.table[0]
                    console.log('退订申请');
                    ...
                    $operation.handleOrderSubmit(unbookData, function(data){
                      if(data.msgCode==0){
        ...
        if($scope.dateString == $scope.compareTime() && a < sessionStorage['lastMealEndTime']*1 && sessionStorage['lastMealEndTime'] < 6*60){
          a += a + 24*60
        }
        if (($scope.dateString == $scope.compareTime()) && (a > b) && $scope.isKuaTian != 1) {
          $scope.showAlert('该餐次已经无法退订');
        } else if ($scope.info.operationType == 1 || checkOrdersAppuser($scope.table, $scope.info.id)) {
              ...
              {text: $T.T('取消')},
              {
                ...
                onTap: function () {
                  if ($scope.data.mvpReason == undefined) {
                    ...
                    if($scope.chooseOrders.length>0){
                      batchno = $scope.chooseOrders[0].getAttribute("data-batchno")
                    }
                    $scope.mvpData = {
                      "businessId": $scope.info.businessId,
                    ...
                    $scope.showLoading();
                    $operation.mvpTab($scope.mvpData, $scope.operationSuccess.mvp);
                  }
                }
              }
            ]
        ...
        if($scope.isTableOrdersMuti()){
          var isMvp = 0
          for (var i = 0; i < $scope.table.length; i++) {
            var tableorders = $scope.table[i].getAttribute('data-orders')?JSON.parse($scope.table[i].getAttribute('data-orders')):[];
            for(var a=0;a<tableorders.length;a++){
        ...
        $scope.goSeatFun = function(){
          ...
          if($scope.chooseOrders.length>0){
            $scope.data.actualNum = $scope.chooseOrders[0].getAttribute("data-resvNum");
          }else{
            $scope.data.actualNum = $scope.table[0].getAttribute("data-resvNum");
          }
          $scope.myPopup = $ionicPopup.confirm({
            cssClass: "er-popup",
            template: '<input type="text" ng-model="data.actualNum" style="background:#f6f6f6;padding:0 10px;">',
            title: $T.T('请输入入座人数'),
            scope: $scope,
            buttons: [
              {text: $T.T('取消')},
              {
                ...
                onTap: function () {
                  ...
                  if($scope.data.actualNum == ''){
                    ...
                    myPopup();
                  } else if (reg.test($scope.data.actualNum) == false || parseInt($scope.data.actualNum) == 0) {
                  ...
                  $scope.showLoading();
                  $operation.status($scope.goSeatData, $scope.operationSuccess.seat, $scope.error);
                }
              }
            ]
          });
        }
        if ((($scope.info.operationType == 1 || $scope.ruzuo) && !$scope.canSubmit) || checkOrdersAppuser($scope.table, $scope.info.id)) {
      ...
      } else if (txt == "锁台" && checkUp($scope.table, 0) && ($scope.lockAuthority) && (!$scope.canSubmit)) {
        var myPopup = $ionicPopup.show({
          cssClass: "er-popup",
          template: '<input type="text" ng-model="data.lock" style="background:#f6f6f6;padding:0 10px;">',
          title: $T.T('请输入锁台原因'),
          scope: $scope,
          buttons: [
            {text: $T.T('取消')},
            {
              ...
              onTap: function () {
                ...
                $scope.showLoading();
                $operation.lock($scope.lockData, $scope.operationSuccess.lock, $scope.error);
              }
            }
          ]
        });
      } else if (txt == "解锁" && checkUp($scope.table, 5) && ($scope.lockAuthority) && (!$scope.canSubmit)) {
          ...
          buttons: [
            {text: $T.T('取消')},
            {
              ...
              onTap: function () {
                ...
                $scope.showLoading();
                $operation.unlock($scope.unlockData, $scope.operationSuccess.unlock, $scope.error);
              }
            }
          ]
        });
      } else if (txt == "换桌" && (checkTableOrders($scope.table, 1)) && ($scope.table.length == 1) && ($scope.huanzuo || ($scope.canSubmit && $scope.huanzhuoSubmit))) {
        if ($scope.info.operationType == 1 || checkOrdersAppuser($scope.table, $scope.info.id)) {
          $scope.changeSFun = function(){
            ...
            if(!$scope.canSubmit){
              ...
              var orderInfo = $scope.chooseOrders.length>0?$scope.chooseOrders[0]:$scope.table[0]
              if(orderInfo.getAttribute("data-appuserid") != $scope.info.id){
                ...
                buttons: [
                  {text: $T.T('取消')},
                  {
                    ...
                    onTap: function () {
                      console.log('换桌申请');
                      var a = (new Date().getHours() * 60) + (new Date().getMinutes() * 1);
                      var b = $scope.newMeal.resvEndTime.slice(0, 2) * 60 + $scope.newMeal.resvEndTime.slice(3) * 1;
                      if($scope.newDate == $scope.compareTime() && a < sessionStorage['lastMealEndTime']*1 && sessionStorage['lastMealEndTime'] < 6*60){
                        a += a + 24*60
                      }
                      if (($scope.newDate == $scope.compareTime()) && (a > b) && $scope.isKuaTian != 1) {
                        ...
                        $operation.handleOrderSubmit({
                          ...
                          mealTypeId: $scope.newMeal.id || $scope.newMeal.mealTypeId,
      ...
      } else if (txt == "加桌" && (checkTableOrders($scope.table, 1) || checkTableOrders($scope.table, 2)) && ($scope.table.length == 1)) {
        if (($scope.info.operationType == 1) || checkOrdersAppuser($scope.table, $scope.info.id)) {
                ...
                buttons: [
                  {text: $T.T('取消')},
                  {
                    ...
                    onTap: function () {
                      ...
                      $operation.handleOrderSubmit(addData, function(data){
                        if(data.msgCode==0){
        ...
        $scope.confirmFun = function(){
          ...
          $scope.confirmData.resvOrderType = $scope.table[0].getAttribute("data-resvOrderType");
          ...
          $scope.confirmData.status = $scope.table[0].getAttribute("data-state");
          $scope.confirmData.isSendConfirmSms = $scope.table[0].getAttribute("data-isSendConfirmSms");
          ...
          $scope.showLoading();
          $operation.confirm($scope.confirmData, $scope.operationSuccess.confirm, $scope.error);
        }
        if (($scope.info.operationType == 1) || checkOrdersAppuser($scope.table, $scope.info.id)) {
          if($scope.isTableOrdersMuti()){
            var htmlDom = ''
        ...
        $scope.payInfo = function(){
          ...
          if($scope.chooseOrders.length > 1 || $scope.table.length > 1){
            ...
            if($scope.chooseOrders.length > 0){
              $scope.payInfoData.tableId = $scope.chooseOrders[0].getAttribute("data-seat");
            }else {
              $scope.payInfoData.tableId = $scope.table[0].getAttribute("data-seat");
            }
          }
          $scope.payInfoData.businessId = $scope.info.businessId;
          $scope.showLoading();
          $operation.payInfo($scope.payInfoData, $scope.payInfoSuccess, $scope.error);
        }
        if (($scope.info.operationType == 1) || checkOrdersAppuser($scope.table, $scope.info.id)) {
          if($scope.isTableOrdersMuti()){
            var htmlDom = ''
  ...
  $scope.payInfoSuccess = function(data){
    if(data && data.code == 200){
      ...
      $scope.billInfo.resvWeekDay = $scope.weekday[(new Date($scope.dateString.replace(/-/g, "/"))).getDay()];
      ...
      $scope.billInfo.tableName =  $scope.table[0].getAttribute("data-tableName");
      $scope.billInfo.billList = data.data.detailList;
      $scope.billInfo.remark = "您的专属管家：" + $scope.info.surname + " " + $scope.info.username;
      ...
      if(data.data.detailList){
        for(var i = 0;i<$scope.billList.length;i++){
          $scope.billInfo.goodsSumNum += Number($scope.billList[i].goodsNumber);
        }
      }
      $scope.showBill = true;
    }else {
  ...
  $scope.payForTable = function(){
    ...
    $scope.showLoading();
    $operation.payOrStore($scope.payData, $scope.payOrStoreSuccess, $scope.error);
  };
  ...
  $scope.openPopover = function ($event, num) {
    if (num == 1) {
      ...
      if (($scope.table.length != 0) && ($scope.table[0].getAttribute('data-isOPer') == 0)) {
  ...
  $scope.choose = function ($event, num) {
    var a = $event.target;
    var txt = a.getAttribute("data-text");
    if ($scope.dateString < $scope.compareTime() && (txt == '退订' || txt == '换桌' || txt == '入座' || txt == '确认' || txt == '加桌' || txt == '锁台' || txt == '解锁' || txt == '结账')) {
      $showAlert.alert('历史订单无法进行该操作');
      return;
    }
    ...
    if(nowTransform){
      ...
      myScrollTop = document.getElementById('table-scroll').getElementsByClassName("scroll")[0].scrollTop;
    }
  ...
  $scope.fencanbie = function (dateString1, dateString2) {
    ...
    if ($scope.info.fcb == true) {
      ...
      var mtc = JSON.parse(sessionStorage['mealTypes']);
      for (var a = 0; a < mtc.length; a++) {
        if (mtc[a].bandEndTime && (mtc[a].usingDate.indexOf(dateString2) != -1)) {
          var mealStart = mtc[a].resvStartTime.slice(0, 2) * 60 + mtc[a].resvStartTime.slice(3) * 1
          var mealBand = mtc[a].bandEndTime.slice(0, 2) * 60 + mtc[a].bandEndTime.slice(3) * 1
          var mealTypeSecondDaySign = 0
          if(mealStart > mealBand){
  ...
  $scope.selected = function ($event) {
    ...
    if ((!$scope.changeSeat) && (!$scope.addSeat)) {
      if (img.getAttribute("data-select") == "1") {
        ...
        var canchoose = false;
        if($scope.canFT){
          var orders = img.getAttribute('data-orders')?JSON.parse(img.getAttribute('data-orders')):[]
          var batchNoArr = [],repeatBatchNoArr=[],new_arr=[]
          $scope.table.map(function(table){
            var tableorders = table.getAttribute('data-orders')?JSON.parse(table.getAttribute('data-orders')):[]
            tableorders.map(function(order){
        ...
        if (canchoose && (oper >= 1) && ($scope.table[0].getAttribute('data-isOper') >= 1)) {
      ...
      if ((img.getAttribute("data-state") != 0 && !$scope.canFT) || img.getAttribute("data-state") == 5 || img.getAttribute("data-isOper") == 0) {
        var confirmPopup = $ionicPopup.confirm({
          cssClass: "er-popup",
          title: $T.T('易订'),
          template: $T.T('此桌无法加桌'),
        ...
        var batchNo = $scope.table[0].getAttribute("data-batchNo");
        var tableAreaName = $scope.table[0].getAttribute("data-tableAreaName");
        ...
        var data = {
          ...
          appUserId: $scope.info.id,
          appUserPhone: $scope.info.username,
          isTableTurnover: JSON.parse(localStorage.getItem('info')).isTableTurnover,
          destTime: JSON.parse($scope.table[0].getAttribute("data-orders"))[0].destTime
        };
        if($scope.chooseOrders.length>0){
          data.batchNo = $scope.chooseOrders[0].getAttribute("data-batchNo")
          data.resvOrder = $scope.chooseOrders[0].getAttribute("data-resvOrder")
          data.destTime = $scope.chooseOrders[0].getAttribute("data-destTime")
        }
        var a = (new Date().getHours() * 60) + (new Date().getMinutes() * 1);
        var b = $scope.resvEndTime.slice(0, 2) * 60 + $scope.resvEndTime.slice(3) * 1;
        if(!$scope.timeInMealTime()){
        ...
        if (($scope.dateString == $scope.compareTime()) && (a > b) && $scope.isKuaTian != 1) {
          ...
          if(state != 0 && $scope.canFT){
            ...
            if(orders.length>0){
              txt += '桌位 '+ tableName + ' 的 ' + desttime.substr(0,desttime.length-1);
            }
            $scope.myPopup = $ionicPopup.show({
              cssClass: "er-popup",
              ...
              buttons: [
                ...
                },
                {
                  ...
                  onTap: function () {
                    ...
                    $scope.showLoading();
                    $operation.addSeat(data, $scope.operationSuccess.addSeat, $scope.error);
                  }
                }
              ]
            });
      ...
      if ((img.getAttribute("data-state") != 0 && !$scope.canFT) || img.getAttribute("data-state") == 5 || img.getAttribute("data-isOper") == 0) {
        ...
        var changeFun = function(){
          ...
          $scope.changeSeatData = {
            ...
            "oldTableAreaName": $scope.table[0].getAttribute("data-tableAreaName"),
            "oldTableAreaId": $scope.table[0].getAttribute("data-tableAreaId"),
            "isTableTurnover": JSON.parse(localStorage.getItem('info')).isTableTurnover,
            "destTime": JSON.parse($scope.table[0].getAttribute("data-orders"))[0].destTime
          };
          if($scope.chooseOrders.length>0){
            $scope.changeSeatData.resvOrder = $scope.chooseOrders[0].getAttribute("data-resvOrder")
            $scope.changeSeatData.destTime = $scope.chooseOrders[0].getAttribute("data-destTime")
          }
          var a = (new Date().getHours() * 60) + (new Date().getMinutes() * 1);
          var b = $scope.resvEndTime.slice(0, 2) * 60 + $scope.resvEndTime.slice(3) * 1;
          if(!$scope.timeInMealTime()){
          ...
          if (($scope.dateString == $scope.compareTime()) && (a > b) && $scope.isKuaTian != 1) {
            ...
            $showAlert.alert('历史订单无法进行该操作');
          }else if (Number(img.getAttribute("data-maxTableNum")) < Number($scope.table[0].getAttribute("data-tableNum"))) {
            $showAlert.alert('订单桌数大于包厢最大桌数,无法换桌');
          }else {
            $scope.showLoading();
            $operation.changeSeat($scope.changeSeatData, $scope.operationSuccess.changeSeat, $scope.error);
          }
        }
        if($scope.canFT){
          var txt = ''
          ...
          if(orders.length>0){
            txt += '桌位 '+img.getAttribute('data-tableName') + ' 的 ' + desttime.substr(0,desttime.length-1);
            $scope.myPopup = $ionicPopup.show({
              cssClass: "er-popup",
              template: `<p>${txt} 已经有预订</p>`,
              title: $T.T('是否继续换桌'),
  ...
  $scope.active = function (type, $event) {
    // 桌位状态
    if (type==='status') {
      if($scope.filterData.status === $event.target.getAttribute('data-text')){
  ...
  $scope.choiceSubmit = function(){
    var reg = /^\d+$/;
    if ((($scope.cData.minPeopleNum != undefined) && ($scope.cData.minPeopleNum != '') && (reg.test($scope.cData.minPeopleNum) == false))
      || (($scope.cData.maxPeopleNum != undefined) && ($scope.cData.maxPeopleNum != '') && (reg.test($scope.cData.maxPeopleNum) == false))) {
      $showAlert.alert('容纳人数应为数字');
      return;
    }
    if ((($scope.cData.minAmount != undefined) && ($scope.cData.minAmount != '') && (reg.test($scope.cData.minAmount) == false))
      || (($scope.cData.maxAmount != undefined) && ($scope.cData.maxAmount != '') && (reg.test($scope.cData.maxAmount) == false))) {
  ...
  $scope.seeDetail = function () {
    ...
    localStorage['scrollTop'] = myScrollTop;
    if ($scope.chakan) {
      if ($scope.table[0] && $scope.table.length == 1 && ($scope.table[0].getAttribute('data-state') != 5)) {
        ...
        var resvOrder = $scope.table[0].getAttribute('data-resvOrder');
        var isChangeTable = $scope.table[0].getAttribute('data-isChangeTable');
        $scope.seatDataFor.tableId = $scope.table[0].getAttribute('data-seat');
        $scope.seatDataFor.maxTableNum = $scope.table[0].getAttribute('data-maxTableNum');
        var orders = $scope.table[0].getAttribute('data-orders')?JSON.parse($scope.table[0].getAttribute('data-orders')):[];
        var resvOrders = [], desttime = [];
        if(orders.length>0){
          orders.map(function(item) {
        ...
        sessionStorage['bookParams'] = JSON.stringify(stateParams)
        $scope.seatDetail[0] = {};
        $scope.seatDetail[0].tableId = $scope.table[0].getAttribute('data-seat');
        $scope.seatDetail[0].tableAreaId = $scope.table[0].getAttribute('data-tableAreaId');
        $scope.seatDetail[0].tableAreaName = $scope.table[0].getAttribute('data-tableAreaName');
        $scope.seatDetail[0].tableName = $scope.table[0].getAttribute('data-tableName');
        $scope.seatDetail[0].maxPeopleNum = $scope.table[0].getAttribute('data-maxPeopleNum');
        $scope.seatDetail[0].maxTableNum = $scope.table[0].getAttribute('data-maxTableNum');
        sessionStorage.removeItem('orderData')
        var type = 4
        if ($scope.info.operationType == 1 || $scope.info.id == $scope.table[0].getAttribute('data-appUserId')) {
          type = 2
        }
        if (resvOrder) {
        ...
        // }
      } else if ($scope.table[0] && ($scope.table.length == 1) && ($scope.table[0].getAttribute('data-state') == 5)) {
        $scope.showAlert(`${$T.T('锁台原因')}：` + $scope.table[0].getAttribute('data-remark'))
      } else {
  ...
  $scope.seatCheckSuccess = function (data) {
    ...
    var chkSuc = function () {
      ...
      for (var a = 0; a < $scope.table.length; a++) {
        var orders = $scope.table[a].getAttribute('data-orders')?JSON.parse($scope.table[a].getAttribute('data-orders')):[]
        var desttime = []
        if(orders.length>0){
          orders.map(function(item) {
            desttime.push(item.destTime)
        ...
        $scope.seatDetail[a] = {};
        $scope.seatDetail[a].tableId = $scope.table[a].getAttribute('data-seat');
        $scope.seatDetail[a].tableAreaId = $scope.table[a].getAttribute('data-tableAreaId');
        $scope.seatDetail[a].tableAreaName = $scope.table[a].getAttribute('data-tableAreaName');
        $scope.seatDetail[a].tableName = $scope.table[a].getAttribute('data-tableName');
        $scope.seatDetail[a].maxPeopleNum = $scope.table[a].getAttribute('data-maxPeopleNum');
        $scope.seatDetail[a].maxTableNum = $scope.table[a].getAttribute('data-maxTableNum');
      }
      var stateParams = {
        seatDataFor: $scope.seatDataFor,
        type: 4
    ...
    if (data.status == 0 && data.meetingStatus == 0) {
      if (data.meetingTableList != "" && data.meetingTableList.length > 0) {
        ...
        for (var i = 0; i < data.meetingTableList.length; i++) {
          txt += `${data.meetingTableList[i].tableName} ${$T.T('所在厅')} (${data.meetingTableList[i].meetingTableName}) ${$T.T('已有')} ${data.meetingTableList[i].cou} ${$T.T('笔待预定宴会订单。详情请询问:')} ${data.meetingTableList[i].content}</br>`;
        }
        $scope.myPopup = $ionicPopup.show({
          cssClass: "er-popup",
          template: '<p>' + txt + '</p>',
          ...
          for (var i = 0; i < data.occTablesInfo.length; i++) {
            if (i == data.occTablesInfo.length - 1) {
              ...
              if (data.occTablesInfo[i].deviceUserName == '') {
                txt += `${data.occTablesInfo[i].appUserName} ${$T.T('正在预订')} ${data.occTablesInfo[i].occTableName} `;
              } else {
                txt += `${data.occTablesInfo[i].deviceUserName} ${$T.T('正在预订')} ${data.occTablesInfo[i].occTableName} `;
              }
            } else {
              occId += data.occTablesInfo[i].occId + ',';
              if (data.occTablesInfo[i].deviceUserName == '') {
                txt += `${data.occTablesInfo[i].appUserName} ${$T.T('正在预订')} ${data.occTablesInfo[i].occTableName}</br> `;
              } else {
                txt += `${data.occTablesInfo[i].deviceUserName} ${$T.T('正在预订')} ${data.occTablesInfo[i].occTableName}</br> `;
              }
            }
          }
          $scope.myPopup = $ionicPopup.show({
      ...
      var txt = ''
      for(var i=0;i<$scope.table.length;i++){
        var orders = $scope.table[i].getAttribute('data-orders')?JSON.parse($scope.table[i].getAttribute('data-orders')):[]
        var desttime = ''
        if(orders.length>0){
          orders.map(function(item) {
            desttime += item.destTime + ','
        ...
        if(orders.length>0){
          txt += '桌位 '+$scope.table[i].getAttribute('data-tableName') + ' 的 ' + desttime.substr(0,desttime.length-1);
        }
      }
      $scope.myPopup = $ionicPopup.show({
        cssClass: "er-popup",
  ...
  $scope.goTableEdit = function(){
    if($scope.info.appOperationSet.indexOf(14) != -1) {
      ...
      stateParams.dateString = $scope.dateString
      sessionStorage['bookParams'] = JSON.stringify(stateParams)
      $state.go('tableEdit', {'tableId': $scope.tableId,'isYan':false,'tableName':$scope.tableName,'tType':$scope.tType,'tTypeId':$scope.tTypeId,'maxPeopleNum':$scope.maxPeopleNum,'maxTableNum':$scope.maxTableNum,'minAmount':$scope.minAmount,'device':$scope.device,'tableRemark':$scope.tableRemark,'tableUrl':$scope.tableUrl});
    }else {
      $showAlert.alert('无权限');
    }
  };
  ...
  $scope.timeInMealTime = function(){ // 判断跨天餐别是否可预订
    var a = (new Date().getHours() * 60) + (new Date().getMinutes() * 1);
    var b = $scope.resvEndTime.slice(0, 2) * 60 + $scope.resvEndTime.slice(3) * 1;
    var c = $scope.resvStartTime.slice(0, 2) * 60 + $scope.resvStartTime.slice(3) * 1;
  ...
  $scope.book = function () {
    ...
    console.log($scope.dateString,$scope.compareTime(),a,b,$scope.isKuaTian)
    if (($scope.dateString == $scope.compareTime()) && (a > b) && $scope.isKuaTian != 1) {
      $scope.showAlert('该餐次已经停止预订');
    } else {
      ...
      if ($scope.table.length == 0) {
        ...
        if ($scope.yuding && !$scope.canSubmit) {
          if (checkUp($scope.table, 0) || (!checkUp($scope.table, 5) && $scope.canFT)) {
            var txt = "";
            for (var i = 0; i < $scope.table.length; i++) {
            ...
            console.log(txt);
            $httpPsd.tableCheck(seatCheck, $scope.seatCheckSuccess, $scope.error);
