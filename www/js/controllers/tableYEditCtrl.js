angular.module('starter.controllers.tableYEditCtrl', []).controller('tableYEditCtrl', function ($scope, $http, $httpPsd, $httpClue, $ionicPopup, $ionicPopover, $showAlert, $state, $stateParams, $ionicLoading, $qupload, $T) {
  $scope.$on('$ionicView.beforeEnter', function (event, viewData) {
    ...
    if($scope.data.roomArea == '无'){
      $scope.data.roomArea = '';
    }
    if($scope.data.floorHeight == '无'){
      $scope.data.floorHeight = '';
    }
  ...
  $scope.saveAll = function(){
    ...
    if (($scope.data.roomArea != '') && ($scope.data.roomArea != '无') && (reg.test($scope.data.roomArea) == false)) {
      $showAlert.alert('面积应为数字');
      return;
    }
    if (($scope.data.floorHeight != '') && ($scope.data.floorHeight != '无') && (reg.test($scope.data.floorHeight) == false)) {
      $showAlert.alert('层高应为数字');
      return;
    }
    if (($scope.data.maxPeopleNum != '') && ($scope.data.maxPeopleNum != '无') && (reg.test($scope.data.maxPeopleNum) == false)) {
      $showAlert.alert('容纳桌数应为数字');
      return;
    }
